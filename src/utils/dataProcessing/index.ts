...
import type { ContextType1, ContextType2, ContextType3, ContextType4, DataFor1, DataFor2, DataFor3, DataFor4, DataFor5 } from './types';
...
export function getDataEntry(dataFor: DataFor4, slugId?: string): Promise<ContextType4>;  

export async function getDataEntry(dataFor: DataFor1 | DataFor2 | DataFor3 | DataFor4 | DataFor5, slugId?: string): Promise<ContextType1 | ContextType2 | ContextType3 | ContextType4> {
  ...
  if (Array.isArray(entry) && entry.length > 0) {
    ...
    if (getDataType(dataFor)==='DataFor4') {
      return getFormatedDataForContext(entry, dataFor as DataFor4); 
    }
  }
